...
@LazySingleton()
class MLService extends CoreService {
  // singleton boilerplate
  static final MLService _mlService = MLService._internal();
  ...
  Future<String?>? _searchResult(List predictedData) async {
    ...
    /// if no faces saved
    // if (employeeData.faceData == null) return null;
    double minDist = 999;
    double currDist = 0.0;
    String? predRes;

    /// search the closest result 👓
    if (employeeData.faceData != null || employeeData.faceData != '') {
      List listFaceData = json.decode(employeeData.faceData!);

      currDist = _euclideanDistance(listFaceData, predictedData);
      if (currDist <= threshold && currDist < minDist) {
        minDist = currDist;
        predRes = employeeData.name;
      }
    } else {
      predRes = null;
    }
